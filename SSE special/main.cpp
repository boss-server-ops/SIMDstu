			...
			for (int j = 0; j < 5; j++)
			{
				...
				for (int k = 0; k < beixiaoyuannum; k++)
				{
					if (biaoji[k] == 1 && k != i && shouxiang(integernum, beixiaoyuanmatrix[i], beixiaoyuannum) == shouxiang(integernum, beixiaoyuanmatrix[k], beixiaoyuannum))
					{
						...
						{
							va = _mm_loadu_si128((__m128i const*)&beixiaoyuanmatrix[i][t]);
							vt = _mm_loadu_si128((__m128i const*)&beixiaoyuanmatrix[k][t]);
							va = _mm_xor_si128(va, vt);
							_mm_storeu_si128((__m128i*)&beixiaoyuanmatrix[i][t], va);
						}
